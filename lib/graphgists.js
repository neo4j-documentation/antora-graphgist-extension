...
const { convert: convertJupyterNotebook, register: registerExtensions } = require('./graphgists/jupyter-converter')
...
function generateJupyterNotebookAttachments(graphGists, logger) {
  const registry = registerExtensions(logger)
  return graphGists.map((graphGist) => {
    const notebookContent = convertJupyterNotebook(graphGist.asciidoc, registry)
    return createJupyerNotebookAttachment(notebookContent, graphGist)
  })
}
